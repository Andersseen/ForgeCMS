...
Browser (admin UI / CmsApiService)
  ...
  → runtime handler                   @forge-cms/runtime handleList(context, { runtime })
      ...
      3. parse limit/offset/where from query params
...
`name`, `init(env)`, `findById(collection, id)`, `findMany({ collection, limit?, offset?, where? })`,
`create(collection, data)`, `update(collection, id, partial)`, `delete(collection, id)`,
`syncSchema(collections)`. Records are `Record<string, unknown>` with a string `id` generated by the
adapter. `where` is equality-only today.
