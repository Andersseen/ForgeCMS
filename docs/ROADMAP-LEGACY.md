...
> **Archived on 2026-09-06.** Historical plan, superseded by [ROADMAP.md](ROADMAP.md).
> Status claims and proposed numbering below reflect earlier planning; do not execute this as an
> active backlog or use it to infer the current published version.
...
revocation more generally remains out of scope, since tokens are intentionally stateless. See
[docs/specs/053-browser-auth-foundation.md](specs/053-browser-auth-foundation.md).

**Update (2026-09-03, spec 054):** the Angular/admin counterpart is also done —
`@forge-cms/angular`'s `ForgeAuthSession` (signals-based session) and `forgeAuthGuard()` (functional
route guard), `@forge-cms/admin`'s `ForgeSignInComponent`/`ForgeSignUpComponent`/
`forgeAdminAuthRoutes()` and `ForgeUsersWorkspaceComponent`, plus a server-side last-admin invariant on
`UsersCollectionAuthAdapter.updateUser`/`deleteUser`. `apps/www` dogfoods the whole layer. Item 028 is
now **fully done except**: `auth: true` on an arbitrary collection, refresh tokens, lockout, password
reset, email verification, and real session revocation — all deliberately deferred, several blocked on 029. See [docs/specs/054-admin-auth-experience.md](specs/054-admin-auth-experience.md).
